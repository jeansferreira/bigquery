version ?= latest
img = registry.neoway.com.br/dataplatform/bigquery:$(version)
imgdev = dataplatform/bigquerydev:$(version)
uid=$(shell id -u $$USER)
gid=$(shell id -g $$USER)
...
build: modcache imagedev
	$(runbuild) go build -v -ldflags "-w -s -X main.Version=$(version)" -o ./cmd/bigquery/bigquery ./cmd/bigquery
